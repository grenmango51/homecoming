...
Like find_flights.py, this scanner respects anti-bot protections by pausing
for human completion if a challenge is presented.
"""
...
def build_search_pairs(
    *,
    ...
    """Generate date pairs matching find_flights.py exactly."""
...
def resolve_browser_executable(requested: str | None) -> str | None:
    """Prefer installed Chrome or Edge on Windows."""
    if requested:
...
def parse_numeric_price(raw: str) -> float | None:
    """Clean and convert price string to float, handling thousands separators and currency symbols."""
...
def duration_minutes(value: str) -> int | None:
    """Parse duration strings like '17h 35m' or '17 t 35 min' into integer minutes."""
...
def parse_itinerary_json(itinerary: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        for opt in deal_options[:5]:
            agent = opt.get("agentName") or opt.get("agents", [{}])[0].get("name") if opt.get("agents") else None
...
def make_observation(
    *,
    ...
    # Also check whole page text for price mentions if status observed
...
async def run(args: argparse.Namespace) -> int:
    ...
    print(f"Scanning {len(pairs)} exact date pairs for Skyscanner in a visible browser.")
        ...
        try:
            for index, (departure, return_date) in enumerate(pairs, start=1):
                ...
                if args.skip_existing and pair_file.exists():
                    try:
                        ...
                        if cached.get("status") == "observed" and cached.get("fetched_at", "").startswith(today_stamp):
                            print(
                                f"[{index}/{len(pairs)}] {departure} -> {return_date} "
                                f"(cached today: €{cached.get('lowest_observed_price_eur')})"
                            )
                            observations.append(cached)
                            write_daily_report(results_dir, observations)
                            write_daily_report(results_dir, observations, args.reference_price)
                            continue
                    except Exception:
                        pass
                ...
                if observation["status"] in {"blocked", "user_action_required"}:
                    print(
                        "Stopping: browser requires human verification. Please resolve and re-run.",
                        file=sys.stderr,
                    )
                    return 2
    ...
    if args.reference_price is not None:
        ...
        elif reference.get("lowest_observed_price_eur") == args.reference_price:
            print(f"\nSUCCESS: Verified reference pair 2026-12-09 -> 2027-01-09 matches expected cheapest €{args.reference_price:.0f}!")
        else:
            print(
                f"\nReference alert: expected €{args.reference_price:.0f}, observed €{reference.get('lowest_observed_price_eur')}.",
                file=sys.stderr,
            )

    print(f"\nDaily Skyscanner report written: {report_json.name}, {report_csv.name}")
    return 0


def parser() -> argparse.ArgumentParser:
    res = argparse.ArgumentParser(description="Human-supervised Skyscanner matrix scanner.")
    res.add_argument("--origin", default="HEL")
    res.add_argument("--dest", default="HAN")
    res.add_argument("--depart-from", default="2026-12-09")
    res.add_argument("--depart-to", default="2026-12-13")
    res.add_argument("--return-from", default="2027-01-05")
    res.add_argument("--return-to", default="2027-01-09")
    ...
    res.add_argument("--min-stay-nights", type=int, default=21)
    res.add_argument("--delay-seconds", type=int, default=4, help="Delay between searches in seconds")
    res.add_argument("--timeout-seconds", type=int, default=35, help="Page load timeout in seconds")
    res.add_argument("--challenge-timeout-seconds", type=int, default=90, help="Wait time for human challenge solver")
    res.add_argument("--skip-existing", action="store_true", help="Skip queries already observed today")
    res.add_argument("--profile-dir", default=str(DEFAULT_PROFILE_DIR))
    res.add_argument("--results-dir", default=str(DEFAULT_RESULTS_DIR))
    res.add_argument("--browser-executable", help="Custom path to Chrome/Edge executable")
    res.add_argument("--reference-price", type=float, default=782.0, help="Expected € price for 9 Dec 2026–9 Jan 2027; recorded as a pass/fail check.")
    return res
